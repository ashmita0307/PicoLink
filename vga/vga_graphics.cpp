...
// Bit masks for 4-bit (two pixels per byte) packing
#define TOPMASK    0x0F  // Preserves lower 4 bits (Even pixel)
#define BOTTOMMASK 0xF0  // Preserves upper 4 bits (Odd pixel)


// 8x8 Pixel Matrix for the Player Spaceship
const uint8_t player_sprite[8] = {
    ...
    0b11111111, // Row 5:  ██████████ 
    0b10100101, // Row 6:  ██ █  █ ██ 
    0b00000000  // Row 7: (Empty baseline)
};

// 8x8 Pixel Matrix for the Alien Invader
const uint8_t alien_sprite[8] = {
    0b10010001, // Row 0:  █  █  █  █
    0b00111010, // Row 1:   █ ██ ██  
    0b01111110, // Row 2:   ████████ 
    0b11011011, // Row 3:  ██ ██ ████
...
void drawPixel(short x, short y, char color) {
    ...
    int index = pixel >> 1;
    color &= 0x0F; // Ensure it stays within a 4-bit range
    if (pixel & 1) {
        vga_data_array[index] = (vga_data_array[index] & TOPMASK) | (color << 4);
    } else {
        vga_data_array[index] = (vga_data_array[index] & BOTTOMMASK) | color;
    }
}
...
int getPixel(short x, short y) {
    ...
    int index = pixel >> 1;
    if (pixel & 1) {
        return (vga_data_array[index] >> 4) & 0x0F;
    } else {
        return vga_data_array[index] & 0x0F;
    }
}
...
void drawSprite(short x, short y, const uint8_t* sprite, char color, int scale) {
    for (int i = 0; i < 8; i++) { 
        ...
        for (int j = 0; j < 8; j++) { 
            ...
            // Bitmask Check (Is the current pixel 1 or 0)
            if (row & (0x80 >> j)) {
